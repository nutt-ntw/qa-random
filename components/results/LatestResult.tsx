  ...
  return (
    <Panel className="p-5 sm:p-6" aria-labelledby="latest-result-heading">
      ...
      <AnimatePresence mode="wait">
        {result ? (
          <motion.div key={result.round} initial={reducedMotion ? false : { opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="flex min-h-24 flex-wrap items-center gap-2.5">
          ...
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid min-h-56 place-items-center rounded-2xl border border-dashed border-white/12 bg-white/[0.02] text-center">
